...
void URLRequestStreamJob::BeforeStartInUI(
  ...
  if (!opts.Get("body", &value) || !value->IsObject()) {
    // If no body is passed, assume an empty body.
    ended_ = true;
    return;
  }

  mate::Dictionary body(isolate, v8::Local<v8::Object>::Cast(value));
  if (!body.Get("on", &value) || !value->IsFunction() ||
      !body.Get("removeListener", &value) || !value->IsFunction()) {
    // if a body is passed but it is not a stream, signal an error.
    ended_ = true;
    errored_ = true;
    return;
  }
