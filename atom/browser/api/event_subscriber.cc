...
struct JSHandlerData {
  JSHandlerData(v8::Isolate *isolate,
              mate::internal::EventSubscriberBase *handler) :
  ...
  mate::internal::EventSubscriberBase *handler_;
};
...
void InvokeCallback(const v8::FunctionCallbackInfo<v8::Value>& info) {
  ...
  handler_data->handler_->EventEmitted(mate::V8ToString(event), &args);
}
...
EventSubscriberBase::EventSubscriberBase(v8::Isolate *isolate,
  ...
  if (g_cached_template.IsEmpty()) {
    g_cached_template = v8::Global<v8::FunctionTemplate>(isolate_,
              v8::FunctionTemplate::New(isolate_, InvokeCallback));
  }
}
...
void EventSubscriberBase::On(const std::string& event_name) {
  ...
  v8::Local<v8::Value> fn = internal::BindFunctionWith(isolate_,
      isolate_->GetCurrentContext(), fn_template->GetFunction(),
      js_handler_data->handle_.Get(isolate_), event);
  js_handlers_.insert(std::make_pair(event_name,
        v8::Global<v8::Value>(isolate_, fn)));
...
void EventSubscriberBase::RemoveAllListeners() {
  ...
  v8::Isolate::Scope isolate_scope(isolate_);
  v8::HandleScope handle_scope(isolate_);
  for (auto it = js_handlers_.begin(); it != js_handlers_.end();) {
...
EventSubscriberBase::RemoveListener(
  ...
  internal::ValueVector args = {
    StringToV8(isolate_, it->first),
    it->second.Get(isolate_)
  };
  internal::CallMethodWithArgs(isolate_,
      v8::Local<v8::Object>::Cast(emitter_.Get(isolate_)), "removeListener",
      &args);
  it->second.Reset();
