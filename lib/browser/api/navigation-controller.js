...
ipcMain.on('ELECTRON_NAVIGATION_CONTROLLER', (event, method, ...args) => {
  event.sender[method](...args)
})

ipcMain.on('ELECTRON_SYNC_NAVIGATION_CONTROLLER', (event, method, ...args) => {
...
const NavigationController = (() => {
  ...
  NavigationController.prototype.getURL = function () {
    return (this.currentIndex === -1) ? '' : this.history[this.currentIndex]
  }
  ...
  NavigationController.prototype.goToOffset = function (offset) {
    if (!this.canGoToOffset(offset)) return

    let pendingIndex = this.currentIndex + offset
  ...
  NavigationController.prototype.getActiveIndex = function () {
    return (this.pendingIndex === -1) ? this.currentIndex : this.pendingIndex
  }
